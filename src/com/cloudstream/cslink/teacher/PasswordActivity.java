...
public class PasswordActivity extends AppLockActivity implements AsyncTaskCompleteListener<String> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (sharedpref.getString("teacher_id", "") == null || sharedpref.getString("teacher_id", "").length() == 0 || sharedpref.getString("teacher_id", "").equalsIgnoreCase("null")) {
    ...
    private void findview() {
        ...
        btnBack = (View) findViewById(R.id.btnbck);
    ...
    private void getMainActivity(String url2) {
        ...
        try {
            ...
            params.put("code", mPinCode);
            params.put("app_version", ApplicationData.getAppVersion(PasswordActivity.this));
            params.put("os", "android");
            ETechAsyncTask task = new ETechAsyncTask(PasswordActivity.this, this, ConstantApi.TEACHER_LOGIN_EMAIL, params);
            task.execute(ApplicationData.main_url + ConstantApi.TEACHER_LOGIN_EMAIL + ".php?");
    ...
    protected void forgot_password() {
        // TODO Auto-generated method stub
        url = ApplicationData.getlanguageAndApi(PasswordActivity.this, ConstantApi.FORGOT_PINCODE_EMAIL)
                + "email=" + _email;
    ...
    public void send_password() {
        if ((mPinCode.length() > 0)) {
            url = ApplicationData.getlanguageAndApi(PasswordActivity.this, ConstantApi.TEACHER_LOGIN_EMAIL)
                    + "email=" + _email
                    + "&code=" + mPinCode;
            getMainActivity(url);
    ...
    public void onTaskComplete(int statusCode, String responseMsg, String webserviceCb, Object tag) {
        if (statusCode == ETechAsyncTask.COMPLETED) {
            try {
                ...
                if (webserviceCb.equalsIgnoreCase(ConstantApi.TEACHER_LOGIN_EMAIL)) {
                    String flag = jObject.getString("flag");

                    if (Integer.parseInt(flag) == 1) {
                        String badge = jObject.getString("psb");
                        String chat_badge = jObject.getString("chb");
                        String internal_badge = jObject.getString("inb");
                        String register_badge = jObject.getString("rgb");
        ...
        protected Void doInBackground(Void... params) {
            ...
            if (myPrefs.getString("jid", "") != null && myPrefs.getString("jid", "").length() > 0) {
                if (myPrefs.getString("jid", "").substring(0, myPrefs.getString("jid", "").lastIndexOf("@")).length() > 0) {
                    ...
                    editor.commit();
                    try {
                        //XMPPMethod.isconnected(PasswordActivity.this);
