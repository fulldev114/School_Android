...
public class TeacherLoginActivity extends Activity {
    ...
    public void login(View v) {
        ...
        if (edtEmail.getText().toString().length() > 0) {
            if (ApplicationData.isValidEmail(edtEmail.getText().toString())) {
                login_url = ApplicationData.getlanguageAndApi(TeacherLoginActivity.this, ConstantApi.VERIFY_BY_EMAIl)
                        + "email="
    ...
    private void getverify(String verify_url) {
        if (!GlobalConstrants.isWifiConnected(TeacherLoginActivity.this)) {
            return;
        }
                                ...
                                if(response.has("update_info")) {
                                    ...
                                    if (job.has("Android")) {
                                        ...
                                        if (forceUpdateApp.equalsIgnoreCase("Yes")) {
                                            Intent intent = new Intent(TeacherLoginActivity.this, UpdateAvailableActivity.class);
                                            startActivity(intent);
                                        } else {
                                            Intent in = new Intent(TeacherLoginActivity.this, VerifyActivity.class);
                                            startActivity(in);

                                            if (isVersionDifferent.equalsIgnoreCase("Yes")) {
                                                Intent in1 = new Intent(TeacherLoginActivity.this, UpdateAvailableActivity.class);
                                                startActivity(in1);
                                            }
                                        }
                                        finish();
...
//         Intent in = new Intent(ParentLoginActivity.this, NewRegister.class);
//         startActivity(in);
//        finish();
